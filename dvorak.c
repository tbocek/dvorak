...
int main(int argc, char *argv[]) {
    ...
    ret_val = libevdev_new_from_fd(fdi, &dev);
    if (ret_val < 0) {
        fprintf(stderr, "Cannot get ev device [%s]: %s.\n", device, strerror(errno));
        return EXIT_FAILURE;
    }

    if (!libevdev_has_event_code(dev, EV_KEY, KEY_X) || 
        !libevdev_has_event_code(dev, EV_KEY, KEY_C) || 
